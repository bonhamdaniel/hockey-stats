...
# Processes given season, addin any games missing from th db
def processSeason(season, added):
	# Gets the json games file for the given season
	gameFile = HockeyScrape.getGames('OHL', season)
	...
	for g in games:
		try:
			...
			if int(g['game_id']) not in added and 'Final' in g['game_status']:
				# Checks for and remedies missig attendance data
				if g['attendance'] == "":
					g['attendance'] = 0
					
				# Adds game to the db
				cursor.execute('INSERT INTO ohl_game (game_id, season_id, game_date, home_team, visiting_team, home_goals, visiting_goals, period, overtime, shootout, attendance) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', (g['game_id'], g['season_id'], g['date_played'], g['home_team'], g['visiting_team'], g['home_goal_count'], g['visiting_goal_count'], g['period'], g['overtime'], g['shootout'], g['attendance']))
...
cursor.execute('SELECT season_id FROM ohl_season')
...
cursor.execute('SELECT game_id FROM ohl_game')
