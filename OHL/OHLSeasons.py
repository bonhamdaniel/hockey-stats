...
# Checks OHL Season JSON file, adding any seasons not already present in the DB
def processSeason():

	# Gets the json seasons file from the OHL website
	seasonFile = HockeyScrape.getSeasons('OHL')
	...
	for s in seasons:
		...
		if int(s['season_id']) not in added:
			...
			try:
				cursor.execute('INSERT INTO ohl_season (season_id, name, career, playoff, start_date, end_date) VALUES (%s, %s, %s, %s, %s, %s)', (s['season_id'], s['season_name'], s['career'], s['playoff'], s['start_date'], s['end_date']))

			except Exception as e:
				print(e)
		else:
			print('Season #', s['season_id'], 'already present in the database')
